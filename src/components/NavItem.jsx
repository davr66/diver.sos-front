...
export default function NavItem({ href, label, Icon,IconActive,bgColor, match }) {
  ...
  return (
    <Link to={href} className="flex flex-col justify-between items-center text-sm max-h-15" aria-current={active ? 'page' : undefined}>
      ...
      <span className={`inline-block text-[.6rem] font-bold leading-none px-2 py-[0.1rem] rounded-full transition-colors duration-200 transform ${active ? 'scale-105' : 'scale-100'}`} style={labelStyle}>{label}</span>
    </Link>
  );
}
