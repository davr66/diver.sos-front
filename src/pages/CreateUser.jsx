...
import { registerUser, getSkills } from "../services/api";
...
export default function CreateUser() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFeedback({ type: 'error', heading: 'Erro ao criar usuário', message: err?.response?.data?.message || 'Não foi possível criar o usuário.' });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex flex-col items-center w-full px-4 py-4">
      ...
      <h1 className="font-[Nunito] font-extrabold text-2xl mb-6">Cadastrar Novo Usuário</h1>
      <form onSubmit={handleSubmit} className="w-full max-w-4xl flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <InputField label="Senha" name="senha" type="password" value={formData.senha} onChange={handleChange} required />
        ...
        <div className="flex gap-4 mt-4">
          <button type="submit" className="flex-1 border-2 border-b-3 border-r-3 rounded-lg py-2 bg-[var(--profile-bg)] font-semibold hover:brightness-95">Cadastrar Usuário</button>
        </div>
      </form>
